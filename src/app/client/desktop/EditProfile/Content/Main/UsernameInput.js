...
export default (props) => {
    ...
    const [checkUsername, { data, error, called, loading }] = useLazyQuery(api.users.checkUsername,{
    ...
    const handleChange = ({ key, e }) => {
        ...
        if (e.target.value.length > 4) {
            ...
            setErrorMessage({
                errorMessage: C.txts.en.editProfile.usernameMessages.lengthError,
    ...
    React.useEffect(() => {
        ...
        if(called && data) {
            ...
            if (!result) {
                setErrorMessage({
                    errorMessage: C.txts.en.editProfile.usernameMessages.available,
                    errorMessageColor: 'green'
                })
                editProfileDispatch({ type: 'ENABLE_DONE_BUTTON' })
            } else {
                setErrorMessage({
                    errorMessage: C.txts.en.editProfile.usernameMessages.alreadyTaken,
                    errorMessageColor: 'red'
                })
                editProfileDispatch({ type: 'DISABLE_DONE_BUTTON' })
            }
    ...
    return <Input label={state.errorMessage || 'Username'} labelColor={state.errorMessageColor} padding='.65rem' value={editProfile.username} onChange={(e) => handleChange({ key: 'username', e })} width={100} margin='.5rem 0 0' placeholder='Username' />
}