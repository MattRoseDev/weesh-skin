...
const handleStatus = (props, auth, user) => {
    if(!auth.token) {
        return <Button background='primary' radius='50rem' color='background' padding='.5rem 1.25rem' fontWeight='bold' to='/login'>Follow</Button>
    }    

    if (props.match.params.username == auth.username) {
        return <Button background='foreground' radius='50rem' color='background' padding='.5rem 1.25rem' fontWeight='bold' to='settings/profile'>Settings</Button>
    }   
