...
export default props => {
    ...
    React.useEffect(() => {
        if (initComponent && weesh.content.length > 0) {
            frontInput.current.value = weesh.content
            setState([`${weesh.content}`])
            setInitComponent(false)
        }
    }, [initComponent, weesh.content])
    ...
    React.useEffect(() => {
        ...
        if (data && weesh.allowShowSuggestions) {
            dispatch({
                type: 'ADD_WEESH',
                data: {
                    suggestionTags: data.exploreAllForUser
    ...
    const handleChange = e => {
        let spacePosition = frontInput.current.value
            .substr(frontInput.current.selectionStart)
            .indexOf(' ')
        spacePosition = spacePosition == -1 ? 0 : spacePosition
        let weeshContent = frontInput.current.value.substr(
            0,
            frontInput.current.selectionStart + spacePosition,
        )
    ...
    return (
        <StyledContainer>
            <StyledInput
                ...
                onInput={handleChange}></StyledInput>
