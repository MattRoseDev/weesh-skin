...
export default props => {
  ...
  React.useEffect(() => {
    if (!state && called && data && nextPage == 1) {
      const response = data.getUserTicketByLinkForUser.message.ticketMessages;
      ...
      setState(response);
      setNextPage(data.getUserTicketByLinkForUser.message.paginate.nextPage);
    }
  }, [data]);
        ...
        state && (
          <>
            <StyledHeader>
              ...
              <StyledAdd onClick={() => setShowAddMessage(!showAddMessage)}>
                <Components.Global.Icon icon="Edit" color={auth.color} />
