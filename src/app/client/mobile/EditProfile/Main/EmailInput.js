...
export default (props) => {
    ...
    const [checkEmail, { data, error, called, loading }] = useLazyQuery(api.users.checkEmail,{
    ...
    React.useEffect(() => {
        ...
        if(called && data) {
            ...
            if (!result) {
                ...
                setErrorMessage({
                    errorMessage: C.txts.en.editProfile.emailMessages.alreadyTaken,
                    errorMessageColor: 'red'
                })
                editProfileDispatch({ type: 'DISABLE_DONE_BUTTON' })
            }
    ...
    return <Input label={state.errorMessage || 'Email'} labelColor={state.errorMessageColor} padding='.65rem' value={editProfile.email} onChange={(e) => handleChange({ key: 'email', e })} width={100} margin='.5rem 0 0' placeholder='Email' />
}