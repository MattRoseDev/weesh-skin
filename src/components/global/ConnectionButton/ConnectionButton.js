...
export default (props) => {
    ...
    const { apiType, dataType, messageType, acceptButtonType, cancelButtonType } = switchAPI(type)
    ...
    React.useEffect(() => {
        ...
        if (data) {
            ...
            switch (result.status) {
                case 0: return setType('FOLLOW')
                case 1: return setType('REQUEST')
                case 2: return setType('FOLLOWING')
            }
        }
    }, [data])
    ...
    return <>
        ...
        {user.id != auth.id && (type == 'FOLLOWING' ? 
            ...
            </Button> : <Button background='blue' color='background' fontWeight='bold' isLoading={loading || undefined} onClick={() => {
            if (type == 'FOLLOWING') toggleDialog(true)
            else handleConnection()
