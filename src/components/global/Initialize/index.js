...
export default () => {
    ...
    const [isUnreadNotification, setIsUnreadNotification] = React.useState(false)
    ...
    const subscriptionNotificationsResponse = useSubscription(api.notifications.add, {
    ...
    React.useEffect(() => {
        if (!isUnreadNotification && notificationsResponse.data) {
            const notifications = notificationsResponse.data.getNotificationsUserForUser.notifications
            notificationDispatch({
                type: 'PUSH_NOTIFICATION',
                data: notifications,
            })
            notifications.length == 0 && notificationDispatch({
                type: 'EMPTY',
            })
        }
    ...
    React.useEffect(() => {
        if (subscriptionNotificationsResponse.data && !isUnreadNotification) {
            notificationDispatch({
                ...
                data: subscriptionNotificationsResponse.data.addNotificationForUser
            })
        }
