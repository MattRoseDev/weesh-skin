...
export default props => {
    ...
    const handleCompleteTag = ({ title }) => {
        let spacePosition = weesh.textarea.current.value
            .substr(weesh.textarea.current.selectionStart)
            .indexOf(' ')
        spacePosition = spacePosition == -1 ? 0 : spacePosition
        let weeshContent = weesh.textarea.current.value.substr(
            0,
            weesh.textarea.current.selectionStart + spacePosition,
        )
